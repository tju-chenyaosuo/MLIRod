...
python mlirfuzz.py \
--seed existing \
--existing_seed_dir /data/exp/long.regression.smith/seeds \
--replace_data_edge /data/src/llvm-project/build/bin/ReplaceDataEdge \
--replace_control_edge /data/src/llvm-project/build/bin/ReplaceControlEdge \
--delete_node /data/src/llvm-project/build/bin/DeleteNode \
--create_node /data/src/llvm-project/build/bin/CreateNode \
--mlir_opt /data/bin/llvm-project/build/bin/mlir-opt \
--optfile opt.txt \
--collector /data/src/llvm-project/build/bin/CollectCoverage \
--clean True > log.txt
...
--replace_data_edge /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/ReplaceDataEdge \
--replace_control_edge /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/ReplaceControlEdge \
--delete_node /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/DeleteNode \
--create_node /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/CreateNode \
--mlir_opt /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/mlir-opt \
--optfile opt.txt \
--collector /data/exp/2023.11.19.final.exp/src/mlirfuzz/build/bin/CollectCoverage \
--clean True > log.txt
"""
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    # OOA arguments
    parser.add_argument("--timeout", type=int, help="seconds, time limitation for each command.", default=10)
    parser.add_argument("--clean", type=bool, help="Clean the original report and seed or not", default=False)
    # arguments in seed pool initialization
    parser.add_argument("--init_size", type=int, help="Size of seed pool.", default=50)
    parser.add_argument("--seed_dir", type=str, help="Seed folder.", default="seeds")
    parser.add_argument("--seed", type=str, help="Method to initialize the seed pool", default="existing")
    ...
    parser.add_argument("--replace_control_edge", type=str, help="Path of mutator control edge modification", default="")
